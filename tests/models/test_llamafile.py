...
class TestCleanupLlamafile(unittest.TestCase):
    """Test suite for the cleanup_llamafile function.

    This class contains unit tests that verify the behavior of the cleanup_llamafile
    ...
    def test_normal_termination(self, mock_popen, mock_killpg, mock_getpgid):
    ...
    @patch("os.killpg")
    @patch("subprocess.Popen")
    def test_force_kill(self, mock_popen, mock_killpg, mock_getpgid):
    ...
    @patch("os.getpgid")
    @patch("subprocess.Popen")
    def test_os_error_fallback(self, mock_popen, mock_getpgid):
